...
from VenomX.core.call import Venom
from VenomX.misc import SUDOERS, db
from VenomX.utils.database import (is_active_chat,
...
from VenomX.utils.inline.play import (panel_markup_1,
                                          stream_markup,
                                          telegram_markup)
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        if "live_" in queued:
            ...
            try:
                await Venom.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Venom.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
